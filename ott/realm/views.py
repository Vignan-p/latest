...
from django.shortcuts import render, redirect
from .models import Payment  # Import your Payment model here
import razorpay
...
def payment_view(request):
    if request.method == 'POST':
        amount = float(request.POST.get('amount')) * 100  # Convert to paisa (100 paisa = 1 INR)
        ...
        try:
            ...
            return render(request, 'payment_form.html', {'error_message': str(e)})

        # Create a payment record in your Django model
        try:
            payment = Payment.objects.create(user=request.user, amount=amount / 100, payout_id=order['id'])
        except Exception as e:
            return render(request, 'payment_form.html', {'error_message': f"Error creating payment: {str(e)}"})

        # Define your Razorpay UPI ID
        razorpay_upi_id = 'your-razorpay-upi-id'

        # You might want to construct a UPI payment URL here based on `razorpay_upi_id`.

        return render(request, 'payment_success.html', {'order_id': order['id'], 'razorpay_upi_url': razorpay_upi_id})
